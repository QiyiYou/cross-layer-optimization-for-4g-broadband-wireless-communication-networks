function [R_MWC_eq_user,R_RD_eq_user,R_MWC_WWF_user,R_RD_WWF_user,Data_MLWDF...
    ,W_voice,W_video,W_BE]...
    =PHY_MLWDF_user_basic(Slot_current,Raw_data,B,N_sc,N_user,T_slot,N_slot,Eb_N0_dB,P_total,N_path,N_channel,U,Delta,user_set_index,N_service_type,N_user_set,N_user_max)

N0=P_total*(T_slot*1e-3/N_sc)./10.^(Eb_N0_dB/10);
...
% for user_set_index=1:N_user_set
    ...
    %     Weight_sum_MWC(user_set_index,:)=sum(W_user);
    %     Weight_sum_RD(user_set_index,:)=sum(W_user);
...
%         scheduling_MLWDF_v10(Slot_current,N_user,N_slot,Raw_data,T_slot,G,U,QoS,N_service);
%     [W_voice,W_video,W_BE,Data_queue,Data_user,Delay_voice_set,Delay_video_set,Delay_BE_set]...
%     =scheduling_MLWDF_example(Slot_current,N_user,N_slot,T_slot,Raw_data,U,Delta);

    %-------------------------------------------------------------------%
    capacity_avg=ones(N_user*N_service_type,1);
    Weight_MLWDF=zeros(N_user*N_service_type,N_slot);
    Data_MLWDF=zeros(N_user*N_service_type,N_slot);
%     [data_MLWDF,delay_MLWDF]=Data_Delay_Generation(N_user,QoS,T_slot*N_slot,U);
%     [Data_MLWDF, Weight_MLWDF]=scheduling_MLWDF_v11(Raw_data,U,Data_MLWDF,N_user,Slot_current,capacity_avg, Weight_MLWDF,T_slot);
    [Data_MLWDF, Weight_MLWDF]=MAC_MLWDF_user_basic_m(Slot_current,Raw_data,Data_MLWDF,Weight_MLWDF,N_user,U,capacity_avg,T_slot,Delta,N_service_type);
    W_voice=Weight_MLWDF(1:N_service_type:end,:);
    W_video=Weight_MLWDF(2:N_service_type:end,:);
    W_BE=Weight_MLWDF(3:N_service_type:end,:);
%     [Data_MLWDF, weight_MLWDF]=scheduling_MLWDF(data_MLWDF,U,Data_MLWDF,K,currentslot, Cap_avg, weight_MLWDF,T);
...
%         weight(k,currentslot)=sum(w_Voice(k,:))+sum(w_Video(k,:))+sum(w_BE(k,:));
        W_user(user_index,Slot_current)=sum(sum(W_voice(user_index,Slot_current))+sum(W_video(user_index,Slot_current))+sum(W_BE(user_index,Slot_current)));
    end
        ...
        while(flag_MWC_WWF_sc~=0)||(flag_RD_WWF_sc~=0)
            ...
            for sc_index=1:N_sc
                ...
                if P_MWC_WWF_sc_iter(cn_index,sc_index)==0
                    ...
                    %P_MC_WF_sc(cn_index,sc_index)=((P_total+sum(rec_r_MWC(cn_index,:)))/num_MC_WF_sc(user_set_index,cn_index))-rec_r_MWC(cn_index,sc_index);
                    %P_MWC_WWF_sc(cn_index,sc_index)=(W_user(MWC_result2(cn_index,sc_index))/sum(W_user)/num_MWC_WWF_sc(user_set_index,cn_index)).*(P_total+sum(rec_r_MWC(cn_index,:)))-rec_r_MWC(cn_index,sc_index);
                    ...
                    %P_MWC_WWF_sc(cn_index,sc_index)=((P_total+sum(rec_r_MWC(cn_index,:)))/num_MWC_WWF_sc(user_set_index,cn_index))-rec_r_MWC(cn_index,sc_index);
                end
%                 PA(pos(t))=(Pt+sum(1./Newindex_CNR))*W(k,currentslot)/sumW-1/index_CNR(pos(t));
                if P_RD_WWF_sc_iter(cn_index,sc_index)==0
                    %P_RD_WWF_sc(cn_index,sc_index)=((P_total+sum(rec_r_RD(cn_index,:)))/num_RD_WWF_sc(user_set_index,cn_index))-rec_r_RD(cn_index,sc_index);
                ...
                if (P_MWC_WWF_sc(cn_index,sc_index)<=0)&&(rec_r_MWC(cn_index,sc_index)~=0)
                    ...
                    %Weight_sum_MWC(user_set_index,cn_index)=Weight_sum_MWC(user_set_index,cn_index)-
                end
...
%     index_CNR(n)=CNR(k,n);
    ...
    %     R_MWC_eq_slot(user_set_index,1)=mean(R_MWC_eq_cn(:));
    %     R_RD_eq_slot(user_set_index,1)=mean(R_RD_eq_cn(:));
    %     R_MWC_WWF_slot(user_set_index,1)=mean(R_MWC_WWF_cn(:));
    %     R_RD_WWF_slot(user_set_index,1)=mean(R_RD_WWF_cn(:));
    %Data rate of users at slots
